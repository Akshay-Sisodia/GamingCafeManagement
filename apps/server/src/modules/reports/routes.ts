...
export async function registerReportRoutes(app: FastifyInstance): Promise<void> {
  app.get("/dashboard", { preHandler: requireUser() }, async (req) => {
    const user = req.user!;
  ...
  app.get("/reports/sessions", { preHandler: requireUser(["owner", "manager"]) }, async (req) => {
    const user = req.user!;
  ...
  app.get("/reports/food", { preHandler: requireUser(["owner", "manager"]) }, async (req) => {
    const user = req.user!;
    const query = req.query as { from?: string; to?: string };
    const from = dayStart(query.from);
    const to = query.to ? new Date(`${query.to}T23:59:59.999Z`) : new Date();
