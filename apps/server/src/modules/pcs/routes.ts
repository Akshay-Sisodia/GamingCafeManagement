...
import { healthReportSchema, uuidv7 } from "@gaming-cafe/shared";
import { z } from "zod";
...
export async function registerPcRoutes(app: FastifyInstance): Promise<void> {
  app.get("/pcs", { preHandler: requireUser() }, async (req) => {
    const user = req.user!;
    const rows = await db
      .select({
        pc: pcs,
  ...
  app.post("/pcs", { preHandler: requireUser(["owner", "manager"]) }, async (req) => {
    const user = req.user!;
  ...
  app.patch("/pcs/:id", { preHandler: requireUser(["owner", "manager"]) }, async (req) => {
    const user = req.user!;
    const id = (req.params as { id: string }).id;
    const input = parseBody(
      z.object({
  ...
  app.post("/agent/health", { preHandler: requireDevice() }, async (req) => {
    ...
    `);
    const rows: Array<{ cafe_id: string; status: string }> = Array.isArray(raw)
      ? (raw as Array<{ cafe_id: string; status: string }>)
      : ((raw as { rows?: Array<{ cafe_id: string; status: string }> }).rows ?? []);
    const row = rows[0];
  ...
  app.get("/pcs/:id/health/latest", { preHandler: requireUser() }, async (req) => {
    const user = req.user!;
    const id = (req.params as { id: string }).id;
  ...
  app.get("/agent/bootstrap", { preHandler: requireDevice() }, async (req) => {
    const device = req.device!;
    ...
    const commandRows = (
      Array.isArray(pendingCommands)
        ...
        : ((pendingCommands as unknown as { rows: Array<Record<string, unknown>> }).rows ?? [])
    ).map((r) => ({
      id: String(r.id),
      pc_id: pcId,
      type: String(r.type),
  ...
  app.get("/agent/time-check", { preHandler: requireDevice() }, async () => {
    return { server_time_ms: Date.now() };
  });

  // Superadmin online verification (docs/01 §5.4): the passcode is an OWNER
  // credential; we verify against owner password hashes for this café.
  const superadminAttempts = new Map<string, { count: number; lockedUntil: number }>();

  app.post("/pcs/:id/superadmin/verify", { preHandler: requireDevice() }, async (req) => {
    const device = req.device!;
    const id = (req.params as { id: string }).id;
    ...
    // Server-side rate limit: 5 fails → 30s lockout (per PC).
    const now = Date.now();
    ...
    for (const owner of ownerRows) {
      try {
        if (await argon2.verify(owner.passwordHash, input.password)) {
          ok = true;
          break;
        }
      } catch {
