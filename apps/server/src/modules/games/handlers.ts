...
export async function handleListGames(req: FastifyRequest) {
  const user = req.user!;
...
export async function handleCreateGame(req: FastifyRequest) {
  const user = req.user!;
...
export async function handlePatchGame(req: FastifyRequest) {
  const user = req.user!;
...
export async function handleCreateGameVersion(req: FastifyRequest) {
  const user = req.user!;
  const id = (req.params as { id: string }).id;
  const input = parseBody(
    z.object({
...
export async function handleListDeployments(req: FastifyRequest) {
  const user = req.user!;
...
export async function handlePauseDeploymentTarget(req: FastifyRequest) {
  const user = req.user!;
  const id = (req.params as { id: string }).id;
  const rows = await db
...
export async function handleResumeDeploymentTarget(req: FastifyRequest) {
  const user = req.user!;
  const id = (req.params as { id: string }).id;
  const rows = await db
    .select({ target: gameDeploymentTargets, cafeId: gameDeployments.cafeId })
...
export async function handleAgentDeploymentsActive(req: FastifyRequest) {
  ...
  const rows = await db
    ...
    .where(
      and(
        ...
        inArray(gameDeploymentTargets.state, [
          "queued",
          "downloading",
          "verifying",
          "installing",
...
export async function handleDeploymentTargetProgress(req: FastifyRequest) {
  const device = req.device!;
...
export async function handleDeploymentTargetComplete(req: FastifyRequest) {
  const device = req.device!;
  const { jobId, pcId } = req.params as { jobId: string; pcId: string };
  if (pcId !== device.pc_id) throw problem(403, "Forbidden", "NOT_YOUR_TARGET");

  const input = parseBody(z.object({ manifest_hash_verified: z.boolean() }), req.body);
  ...
  const updated = await db
    ...
    .where(
      and(eq(gameDeploymentTargets.deploymentId, jobId), eq(gameDeploymentTargets.pcId, pcId)),
    )
    .returning();
  const target = updated[0]!;
...
export async function handleDeploymentTargetFail(req: FastifyRequest) {
  const device = req.device!;
  const { jobId, pcId } = req.params as { jobId: string; pcId: string };
  if (pcId !== device.pc_id) throw problem(403, "Forbidden", "NOT_YOUR_TARGET");
